    ...
    const fetchProductData = async () => {
      try {
        ...
        const klow = data.products.find(
          (product: any) => product.slug === "klow-10mg"
        );

        setProductData({
          inventory: Number(klow?.inventory ?? 0),
          price: Number(klow?.price ?? 55),
        });
      } catch (error) {
      ...
      <section className="px-6 md:px-10 pb-16">
        <div className="max-w-7xl mx-auto rounded-[36px] border border-white/10 bg-white/[0.04] backdrop-blur-sm p-8 md:p-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            {[
              ["Peptide Research", "Studied in controlled peptide-based laboratory models."],
              ["Metabolic Pathways", "Evaluated in models involving metabolic signaling and pathway research."],
              ["Receptor Activity", "Frequently researched in receptor-related laboratory studies."],
